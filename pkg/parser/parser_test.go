...
var resolveTests = []caseTest{
	{
		name: "Test JSON Resolver",
		args: resolveArgs{
			filename: "test.json",
			content: []byte(`
{
...
// TestParser_Resolve tests the functions [Resolve()] and all the methods called by them
func TestParser_Resolve(t *testing.T) {
	p := initilizeBuilder()
	for _, tt := range resolveTests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := p.Resolve(tt.args.content, tt.args.filename)
			require.NoError(t, err)
