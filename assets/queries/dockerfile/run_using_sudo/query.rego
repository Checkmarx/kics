...
CxPolicy[result] {
	...
	from_command := dockerLib.get_original_from_command(stage)
	run_command := substring(resource.Original, 0, 3)
	run_value := trim_space(trim_prefix(resource.Original, run_command))
	result := {
		"documentId": input.document[i].id,
		"searchKey": dockerLib.add_line_hint(sprintf("%s={{%s}}.%s={{%s}}", [from_command.Value, name, run_command, run_value]), from_command.LineHint),
		"issueType": "IncorrectValue",
		"keyExpectedValue": "RUN instruction shouldn't contain sudo",
		"keyActualValue": "RUN instruction contains sudo",
	}
