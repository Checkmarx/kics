...
CxPolicy [ result ] {
	...
	not regex.match("{ *\\(\\$\\.eventSource *= *s3\\.amazonaws\\.com\\) *&& *\\(\\(\\$\\.eventName *= *PutBucketAcl\\) *\\|\\| *\\(\\$\\.eventName *= *PutBucketPolicy\\) *\\|\\| *\\(\\$\\.eventName *= *PutBucketCors\\) *\\|\\| *\\(\\$\\.eventName *= *PutBucketLifecycle\\) *\\|\\| *\\(\\$\\.eventName *= *PutBucketReplication\\) *\\|\\| *\\(\\$\\.eventName *= *DeleteBucketPolicy\\) *\\|\\| *\\(\\$\\.eventName *= *DeleteBucketCors\\) *\\|\\| *\\(\\$\\.eventName *= *DeleteBucketLifecycle\\) *\\|\\| *\\(\\$\\.eventName *= *DeleteBucketReplication\\)\\) *}",
