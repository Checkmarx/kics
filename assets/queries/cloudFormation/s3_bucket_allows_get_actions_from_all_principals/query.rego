...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("Resources.%s.Properties.AssumeRolePolicyDocument.Statement does not allow a 'Get' action from all principals", [name]),
		"keyActualValue": sprintf("Resources.%s.Properties.AssumeRolePolicyDocument.Statement allows a 'Get' action from all principals", [name]),
	}
}
