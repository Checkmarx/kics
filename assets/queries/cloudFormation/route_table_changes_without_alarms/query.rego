...
CxPolicy[result] {
	...
	not regex.match("{ *(\\(\\$\\.eventName *= *AssociateRouteTable\\) *\\|\\|)? *\\(\\$\\.eventName *= *CreateRoute\\) *\\|\\| *\\(\\$\\.eventName *= *CreateRouteTable\\) *\\|\\| *\\(\\$\\.eventName *= *ReplaceRoute\\) *\\|\\| *\\(\\$\\.eventName *= *ReplaceRouteTableAssociation\\) *\\|\\| *\\(\\$\\.eventName *= *DeleteRouteTable\\) *\\|\\| *\\(\\$\\.eventName *= *DeleteRoute\\) *\\|\\| *\\(\\$\\.eventName *= *DisassociateRouteTable\\) *}", resourceMetricFilter[name].Properties.FilterPattern)
