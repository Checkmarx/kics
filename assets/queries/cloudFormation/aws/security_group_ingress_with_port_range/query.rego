...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("Resources.%s.Properties.FromPort should be equal to Resources.%s.Properties.ToPort", [name, name]),
...
CxPolicy[result] {
	...
	result := {
		...
		"issueType": "IncorrectValue",
		"keyExpectedValue": sprintf("Resources.%s.Properties.SecurityGroupIngress[%d].FromPort should be equal to Resources.%s.Properties.SecurityGroupIngress[%d].ToPort", [name, index, name, index]),
