...
isAccessibleFromEntireNetwork(ingress) = accessible {
	endswith(ingress.CidrIp, "/0")
  accessible = true
}

containsDestinationPort(port, ingress) = containing {
    portStart = ingress.FromPort
    portEnd = ingress.ToPort
    portStart <= port
    portEnd >= port
    containing = true
} 
...
#############	Result
  result := {
        ...
        "keyExpectedValue": sprintf("%s (%s:%d) should not be allowed in load balancer %s", [portName, protocol, portNumber, name]),
        "keyActualValue":   sprintf("%s (%s:%d) is allowed in load balancer %s", [portName, protocol, portNumber, name])
            }
}
