...
CxPolicy[result] {
	...
	not regex.match("{ *\\(\\$\\.eventName *= *CreateTrail\\) *\\|\\| *\\(\\$\\.eventName *= *UpdateTrail\\) *\\|\\| *\\(\\$\\.eventName *= *DeleteTrail\\) *\\|\\| *\\(\\$\\.eventName *= *StartLogging\\) *\\|\\| *\\(\\$\\.eventName *= *StopLogging\\) *}", resourceMetricFilter[name].Properties.FilterPattern)
