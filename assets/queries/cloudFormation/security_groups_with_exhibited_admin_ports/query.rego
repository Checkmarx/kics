...
CxPolicy [ result ] {
   ...
   check_security_groups_egress(resources.Properties, ports)
   
      result := {
                "documentId": 		input.document[i].id,
                "searchKey":        sprintf("Resources.%s.Properties.SecurityGroupEgress", [name]),
                "issueType":		   "IncorrectValue",
                "keyExpectedValue": sprintf("None of the Resources.%s.Properties.SecurityGroupEgress has a correct port", [name]),
                "keyActualValue": 	sprintf("One of the Resources.%s.Properties.SecurityGroupEgress has a too exposed port", [name])
              }
}
...
check_security_groups_egress(group, ports) {
    some p
	group.SecurityGroupEgress[_].FromPort == ports[p]
}

check_security_groups_egress(group, ports) {
    some p
	group.SecurityGroupEgress[_].ToPort == ports[p]
}