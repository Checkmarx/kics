...
CxPolicy[result] {
	...
	not regex.match("{ *\\(\\$\\.eventName *= *AuthorizeSecurityGroupIngress\\) *\\|\\| *\\(\\$\\.eventName *= *AuthorizeSecurityGroupEgress\\) *\\|\\| *\\(\\$\\.eventName *= *RevokeSecurityGroupIngress\\) *\\|\\| *\\(\\$\\.eventName *= *RevokeSecurityGroupEgress\\) *\\|\\| *\\(\\$\\.eventName *= *CreateSecurityGroup\\) *\\|\\| *\\(\\$\\.eventName *= *DeleteSecurityGroup\\) *}", resourceMetricFilter[name].Properties.FilterPattern)
