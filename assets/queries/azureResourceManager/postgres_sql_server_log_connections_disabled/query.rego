...
CxPolicy[result] {
	...
	endswith(childValue.name, "log_connections")
	childValue.properties.value == "off"

	result := {
		"documentId": input.document[i].id,
		"searchKey": sprintf("%s.name={{%s}}.%s.name=log_connections.properties.value", [common_lib.concat_path(parentPath), parentName, common_lib.concat_path(childPath)]),
		"issueType": "IncorrectValue",
		"keyExpectedValue": "child resource with 'configurations' of resource type 'Microsoft.DBforPostgreSQL/servers' has 'log_connections' set to 'off'",
		"keyActualValue": "child resource with 'configurations' of resource type 'Microsoft.DBforPostgreSQL/servers' has 'log_connections' set to 'on'",
	}
}
...
CxPolicy[result] {
	...
	[childPath, childValue] := walk(parentValue)
	childValue.type == "configurations"
	not endswith(childValue.name, "log_connections")
...
CxPolicy[result] {
	...
	result := {
		...
		"issueType": "IncorrectValue",
		"keyExpectedValue": "resource with type 'Microsoft.DBforPostgreSQL/servers/configurations' has 'log_connections' property set to 'off'",
