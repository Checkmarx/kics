...
CxPolicy [ result ] {
  ...
  result := {
                ...
                "keyExpectedValue": sprintf("'name={{%s}}.rules.cidr_ip' is one of [10.0.0.0/8, 192.168.0.0/16, 172.16.0.0/12]", [ec2_instanceName,j]),
...
checkPrivateIps(ipVal){
    ...
    net.cidr_contains(private_ips[e], ipVal)
}
