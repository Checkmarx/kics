...
getForProvider(apiVersion, kind, name, docs, file) = forProvider {
    doc := docs[_]
    [_, resource] := walk(doc)
...
existsInternetGateway(dbSubnetGroupName, file) {
    DBSGforProvider := getForProvider("database.aws.crossplane.io", "DBSubnetGroup", dbSubnetGroupName, input.document, file)
    subnetIds := DBSGforProvider.subnetIds
    count(subnetIds) > 0
    subnetId := subnetIds[s]

    EC2SforProvider := getForProvider("ec2.aws.crossplane.io", "Subnet", subnetId, input.document, file)
...
CxPolicy[result] {
    ...
    existsInternetGateway(dbSubnetGroupName, resource.file) == true
