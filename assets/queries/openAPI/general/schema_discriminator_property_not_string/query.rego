...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("%s.{{%s}} should be set to string", [openapi_lib.concat_path(path), discriminator.path]),
		"keyActualValue": sprintf("%s.{{%s}} is not set to string", [openapi_lib.concat_path(path), discriminator.path]),
		"overrideKey": version,
	}
}
