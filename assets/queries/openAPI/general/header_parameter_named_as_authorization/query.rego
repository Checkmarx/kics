...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("%s.parameters.name={{%s}} is not 'Authorization'", [searchKey, params.name]),
		"keyActualValue": sprintf("%s.parameters.name={{%s}} is 'Authorization'", [searchKey, params.name]),
		"overrideKey": version,
	}
}
