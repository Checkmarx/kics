...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("%s.{{%s}}.%s is set in 'properties'", [openapi_lib.concat_path(path), discriminator.path]),
...
CxPolicy[result] {
	...
	result := {
		...
		"issueType": "IncorrectValue",
		"keyExpectedValue": sprintf("%s.{{%s}}.%s is set in 'properties'", [schemaInfo.path, s, discriminator.path]),
