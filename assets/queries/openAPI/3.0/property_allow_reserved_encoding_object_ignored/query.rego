...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("paths.{{%s}}.{{%s}}.requestBody.content.{{%s}} should be 'application/x-www-form-urlencoded' when 'allowReserved' is set", [path, operation, x]),
...
CxPolicy[result] {
	...
	result := {
		...
		"issueType": "IncorrectValue",
		"keyExpectedValue": sprintf("components.requestBodies.{{%s}}.content.{{%s}} should be 'application/x-www-form-urlencoded' when 'allowReserved' is set", [r, x]),
...
improperly_defined(content, x) {
	...
	common_lib.valid_key(content.encoding[e], "allowReserved")
}
