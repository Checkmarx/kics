...
CxPolicy[result] {
	...
	url := doc.paths[path][operation].requestBodies[r].content[c].schema.externalDocs.url
	not openapi_lib.is_valid_url(url)

	result := {
		"documentId": doc.id,
		"searchKey": sprintf("paths.{{%s}}.{{%s}}.requestBodies.{{%s}}.content.{{%s}}.schema.externalDocs.url", [path, operation, r, c]),
		"issueType": "IncorrectValue",
		"keyExpectedValue": sprintf("paths.{{%s}}.{{%s}}.requestBodies.{{%s}}.content.{{%s}}.schema.externalDocs.url has a valid URL", [path, operation, r, c]),
		"keyActualValue": sprintf("paths.{{%s}}.{{%s}}.requestBodies.{{%s}}.content.{{%s}}.schema.externalDocs.url has a invalid URL", [path, operation, r, c]),
	}
}
