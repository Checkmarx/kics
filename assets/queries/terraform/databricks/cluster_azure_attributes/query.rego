...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("'databricks_cluster[%s].azure_attributes.availability' should not equal to 'SPOT'", [name]),
...
CxPolicy[result] {
	...
	result := {
		...
		"issueType": "IncorrectValue",
		"keyExpectedValue": sprintf("'databricks_cluster[%s].azure_attributes.first_on_demand' should not equal to '0'", [name]),
