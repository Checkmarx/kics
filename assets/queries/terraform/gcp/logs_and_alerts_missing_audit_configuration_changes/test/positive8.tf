resource "google_logging_metric" "audit_config_change" {
  ...
  filter      = "protoPayload.methodName=\"SetIamPolicy\" OR protoPayload.serviceData.policyDelta.auditConfigDeltas:*"
}
...
resource "google_monitoring_alert_policy" "audit_config_alert" {
  ...
  conditions {
    ...
    condition_threshold {
      filter = "resource.type=\"gce_instance\" OR metric.type=\"logging.googleapis.com/user/audit_config_change\""
    }
  }
