resource "google_logging_metric" "audit_config_change" {
  ...
  filter      = "NOT (NOT protoPayload.serviceData.policyDelta.auditConfigDeltas:* OR protoPayload.methodName=\"SetIamPolicy\")"
}
...
resource "google_monitoring_alert_policy" "audit_config_alert" {
  ...
  conditions {
    ...
    condition_matched_log {
      filter = <<-FILTER
      NOT (NOT protoPayload.serviceData.policyDelta.auditConfigDeltas : *
