...
get_results(resource, name) = results {
	...
	results := {
		...
		"keyExpectedValue": sprintf("'google_sql_database_instance[%s].settings.database_flags' should be defined and set '3625' to 'on'", [name]),
	...
	results := {
		...
		"issueType": "MissingAttribute",
		"keyExpectedValue": sprintf("'google_sql_database_instance[%s].settings.database_flags' should be defined and set '3625' to 'on'", [name]),
		"keyActualValue": sprintf("'google_sql_database_instance[%s].settings.database_flags' does not set '3625'", [name]),
		"searchLine": common_lib.build_search_line(["resource", "google_sql_database_instance", name, "settings", "database_flags"], [])
	}

} else = results { # array
	common_lib.valid_key(resource, "settings")
	resource.settings.database_flags[x].name  == "3625"
	...
	results := {
		...
		"keyExpectedValue": sprintf("'google_sql_database_instance[%s].settings.database_flags' should be defined and set '3625' to 'on'", [name]),
		"keyActualValue": sprintf("'google_sql_database_instance[%s].settings.database_flags' sets '3625' to '%s'", [name, resource.settings.database_flags[x].value]),
	...
	common_lib.valid_key(resource, "settings")
	resource.settings.database_flags.name  == "3625"
	...
	results := {
		...
		"issueType": "IncorrectValue",
		"keyExpectedValue": sprintf("'google_sql_database_instance[%s].settings.database_flags' should be defined and set '3625' to 'on'", [name]),
		"keyActualValue": sprintf("'google_sql_database_instance[%s].settings.database_flags' sets '3625' to '%s'", [name, resource.settings.database_flags.value]),
