...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("'google_sql_database_instance[%s].settings.database_flags' should be defined and set 'user connections' to '0'", [name]),
		"keyActualValue": sprintf("'google_sql_database_instance[%s].settings.database_flags' sets 'user connections' to '%s'", [name, results.value]),
		"searchLine": results.searchLine
	}
}

get_results(resource, name) = results { # array
	resource.settings.database_flags[x].name == "user connections"
	...
	resource.settings.database_flags.name == "user connections"
