...
CxPolicy[result] {
    ...
    unrecommended_permission_policy_scenarios(targetUser, "iam:AttachRolePolicy")
	...
	result := {
        ...
        "keyExpectedValue": sprintf("user %s is not associated with a policy that has Action set to 'iam:AttachRolePolicy' and Resource set to '*'", [targetUser]),
		"keyActualValue": sprintf("user %s is associated with a policy that has Action set to 'iam:AttachRolePolicy' and Resource set to '*'", [targetUser]),
