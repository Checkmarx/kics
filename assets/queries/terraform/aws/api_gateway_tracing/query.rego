...
CxPolicy [ result ] {
  ...
  resource.xray_tracing_enabled == false
	...
	result := {
                ...
                "keyExpectedValue": sprintf("'aws_api_gateway_stage[%s].xray_tracing_enabled' is false", [name]),
                "keyActualValue": 	sprintf("'aws_api_gateway_stage[%s].xray_tracing_enabled' is true", [name])
              }
}
