...
CxPolicy [ result ] {
  ...
  check_role(statement.Action, "*") == true
	...
	result := {
                ...
                "keyExpectedValue": sprintf("aws_s3_bucket[%s].policy doesn't make the bucket accesible to all AWS Accounts", [name]),
                "keyActualValue": 	sprintf("aws_s3_bucket[%s].policy does make the bucket accesible to all AWS Accounts", [name]),
              }
}
