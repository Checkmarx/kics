...
CxPolicy[result] {
    ...
    unrecommended_permission_policy_scenarios(targetUser, "iam:PutUserPolicy")
	...
	result := {
        ...
        "keyExpectedValue": sprintf("user %s is not associated with a policy that has Action set to 'iam:PutUserPolicy' and Resource set to '*'", [targetUser]),
		"keyActualValue": sprintf("user %s is associated with a policy that has Action set to 'iam:PutUserPolicy' and Resource set to '*'", [targetUser]),
