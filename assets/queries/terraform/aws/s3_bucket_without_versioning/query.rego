...
CxPolicy[result] {
    ...
    not has_associated_versioning(bucketName, input.document[i]) # version after TF AWS 4.0
...
has_associated_versioning(aws_s3_bucket_name, doc) {
	[_, value] := walk(doc)
	bucket_versioning := value.aws_s3_bucket_versioning[_]
	contains(bucket_versioning.bucket, sprintf("aws_s3_bucket.%s", [aws_s3_bucket_name]))
}
