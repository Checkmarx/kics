...
CxPolicy[result] {
    ...
    unrecommended_permission_policy_scenarios(targetUser, "lambda:CreateFunction")
    unrecommended_permission_policy_scenarios(targetUser, "iam:PassRole")
        unrecommended_permission_policy_scenarios(targetUser, "lambda:InvokeFunction")
	...
	result := {
        ...
        "keyExpectedValue": sprintf("user %s is not associated with a policy that has Action set to 'lambda:CreateFunction' and 'iam:PassRole' and 'lambda:InvokeFunction' and Resource set to '*'", [targetUser]),
		"keyActualValue": sprintf("user %s is associated with a policy that has Action set to 'lambda:CreateFunction' and 'iam:PassRole' and 'lambda:InvokeFunction' and Resource set to '*'", [targetUser]),
