...
CxPolicy[result] {
    ...
    unrecommended_permission_policy_scenarios(targetGroup, "ec2:RunInstances")
	...
	result := {
        ...
        "keyExpectedValue": sprintf("group %s is not associated with a policy that has Action set to 'ec2:RunInstances' and 'iam:PassRole' and Resource set to '*'", [targetGroup]),
		"keyActualValue": sprintf("group %s is associated with a policy that has Action set to 'ec2:RunInstances' and 'iam:PassRole' and Resource set to '*'", [targetGroup]),
