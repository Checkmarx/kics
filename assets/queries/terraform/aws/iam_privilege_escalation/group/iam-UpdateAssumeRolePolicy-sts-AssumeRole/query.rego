...
CxPolicy[result] {
    ...
    unrecommended_permission_policy_scenarios(targetGroup, "iam:UpdateAssumeRolePolicy")
    unrecommended_permission_policy_scenarios(targetGroup, "sts:AssumeRole")
	...
	result := {
        ...
        "keyExpectedValue": sprintf("group %s is not associated with a policy that has Action set to 'iam:UpdateAssumeRolePolicy' and 'sts:AssumeRole' and Resource set to '*'", [targetGroup]),
		"keyActualValue": sprintf("group %s is associated with a policy that has Action set to 'iam:UpdateAssumeRolePolicy' and 'sts:AssumeRole' and Resource set to '*'", [targetGroup]),
