...
CxPolicy[result] {
    ...
    unrecommended_permission_policy_scenarios(targetRole, "iam:AttachRolePolicy")
	...
	result := {
        ...
        "keyExpectedValue": sprintf("role %s is not associated with a policy that has Action set to 'iam:AttachRolePolicy' and Resource set to '*'", [targetRole]),
		"keyActualValue": sprintf("role %s is associated with a policy that has Action set to 'iam:AttachRolePolicy' and Resource set to '*'", [targetRole]),
