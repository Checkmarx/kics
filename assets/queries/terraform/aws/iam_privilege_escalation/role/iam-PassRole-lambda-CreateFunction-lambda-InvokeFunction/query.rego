...
CxPolicy[result] {
    ...
    unrecommended_permission_policy_scenarios(targetRole, "lambda:CreateFunction")
    unrecommended_permission_policy_scenarios(targetRole, "iam:PassRole")
    unrecommended_permission_policy_scenarios(targetRole, "lambda:InvokeFunction")
	...
	result := {
        ...
        "keyExpectedValue": sprintf("role %s is not associated with a policy that has Action set to 'lambda:CreateFunction' and 'iam:PassRole' and 'lambda:InvokeFunction' and Resource set to '*'", [targetRole]),
		"keyActualValue": sprintf("role %s is associated with a policy that has Action set to 'lambda:CreateFunction' and 'iam:PassRole' and 'lambda:InvokeFunction' and Resource set to '*'", [targetRole]),
