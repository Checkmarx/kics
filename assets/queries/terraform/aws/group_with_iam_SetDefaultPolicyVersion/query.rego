...
CxPolicy[result] {
    ...
    unrecommended_permission_policy_scenarios(targetGroup, "iam:SetDefaultPolicyVersion")
	...
	result := {
        ...
        "keyExpectedValue": sprintf("group %s is not associated with a policy that has Action set to 'iam:SetDefaultPolicyVersion' and Resource set to '*'", [targetGroup]),
		"keyActualValue": sprintf("group %s is associated with a policy that has Action set to 'iam:SetDefaultPolicyVersion' and Resource set to '*'", [targetGroup]),
