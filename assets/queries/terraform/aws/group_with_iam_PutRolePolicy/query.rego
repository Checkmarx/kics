...
CxPolicy[result] {
    ...
    unrecommended_permission_policy_scenarios(targetGroup, "iam:PutRolePolicy")
	...
	result := {
        ...
        "keyExpectedValue": sprintf("group %s is not associated with a policy that has Action set to 'iam:PutRolePolicy' and Resource set to '*'", [targetGroup]),
		"keyActualValue": sprintf("group %s is associated with a policy that has Action set to 'iam:PutRolePolicy' and Resource set to '*'", [targetGroup]),
