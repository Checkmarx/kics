...
CxPolicy[result] {
	...
	result := {
		...
		"searchValue": sprintf("%s/%s/%d", [role_id, inline_policy_id, st]),
		...
		"searchLine": common_lib.build_search_line(["resource", "aws_lambda_function", function_id], ["role"]),
	}
}
...
CxPolicy[result] {
	...
	attachment := document[_].resource[attachments[at]][attachment_id]
	...
	result := {
		...
		"resourceName": tf_lib.get_resource_name(lambda, function_id),
		"searchKey": sprintf("aws_lambda_function[%s].role", [function_id]),
		"searchValue": sprintf("%s/%s[%s]/%s/%d", [role_id, attachments[at], attachment_id, attached_customer_managed_policy_id, st]),
		"issueType": "IncorrectValue",
...
CxPolicy[result] {
	...
	result := {
		...
		"resourceName": tf_lib.get_resource_name(lambda, function_id),
		"searchKey": sprintf("aws_lambda_function[%s].role", [function_id]),
		"searchValue": sprintf("%s/%s[%s]", [role_id, attachments[at], attachment_id]),
		"issueType": "IncorrectValue",
