...
CxPolicy[result] {
	...
	result := {
		...
		"searchKey": sprintf("%s[%s].rule[%d].verbs.%s", [resourceTypes[t], name, ru, create]),
		"searchValue": pods,
...
CxPolicy[result] {
	...
	result := {
		...
		"resourceType": resourceTypes[t],
		"resourceName": tf_lib.get_resource_name(resource, name),
		"searchKey": sprintf("%s[%s].rule[%d].verbs.%s", [resourceTypes[t], name, ru, verb]),
		"searchValue": pods,
		"issueType": "IncorrectValue",
