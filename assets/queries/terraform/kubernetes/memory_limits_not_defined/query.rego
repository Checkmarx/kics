...
CxPolicy[result] {
	...
	result := {
		...
		"searchKey": sprintf("%s[%s].%s.%s.name={{%s}}.resources.limits", [resourceType, name, specInfo.path, types[x], containers[y].name]),
		...
		"searchLine": common_lib.build_search_line(["resource", resourceType, name, specInfo.path, types[x], y], ["resources", "limits"]),
	}
}
