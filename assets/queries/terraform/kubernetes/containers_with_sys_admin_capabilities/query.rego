...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("%s[%s].%s.%s[%d].security_context.capabilities.add does not have 'SYS_ADMIN'", [resourceType, name, specInfo.path, types[x], y]),
...
CxPolicy[result] {
	...
	result := {
		...
		"issueType": "IncorrectValue",
		"keyExpectedValue": sprintf("%s[%s].%s.%s.security_context.capabilities.add does not have 'SYS_ADMIN'", [resourceType, name, specInfo.path, types[x]]),
