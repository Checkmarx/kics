...
# No existe NINGUNA regla en el proyecto que monitoree Route Tables.
CxPolicy[result] {
...
# La regla existe, pero le faltan eventos (ej: tiene create pero falta update).
CxPolicy[result] {
...
# La regla es relevante (Route Table) pero está apagada.
CxPolicy[result] {
    rule := input.document[i].resource.oci_events_rule[name]
