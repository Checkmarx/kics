...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": "'azurerm_key_vault' should be associated with 'azurerm_monitor_diagnostic_setting' resources that log the 'audit' and 'allLogs' category groups to valid destinations",
...
get_results(resource, diagnostic_resources) = keyActualValue {		# not associated with a single diagnostic setting resource
	...
	keyActualValue := "'azurerm_key_vault' is not associated with an 'azurerm_monitor_diagnostic_setting' resource"
	...
	keyActualValue := sprintf("'azurerm_databricks_workspace' is associated with %d 'azurerm_monitor_diagnostic_setting' resource(s), but is missing logs for %d category group(s): '%v'",[count(diagnostic_resources), count(missing_logs) ,concat("', '", missing_logs)])
}
...
get_valid_logs(resource) = valid_logs {													# "enabled_log" array
	...
	valid_logs := {x | x := resource.enabled_log[i].category_group
					   resource.enabled_log[i].category_group == required_logs[_]}
} else = valid_logs {																	# "enabled_log" object
	common_lib.valid_key(resource, "enabled_log")
	valid_destination(resource)
	resource.enabled_log.category_group == required_logs[_]
	valid_logs := {resource.enabled_log.category_group}
} else = valid_logs {																			# "log" array
	is_array(resource.log)
	valid_destination(resource)
	valid_logs := {x | x := resource.log[i].category_group
					   resource.log[i].category_group == required_logs[_]
					   is_enabled(resource.log[i])}
} else = valid_logs {																			# "log" object
	common_lib.valid_key(resource, "log")
	valid_destination(resource)
	resource.log.category_group == required_logs[_]
	is_enabled(resource.log)
	valid_logs := {resource.log.category_group}
}
