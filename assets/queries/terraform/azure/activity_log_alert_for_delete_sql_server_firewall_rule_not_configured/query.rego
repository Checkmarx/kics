...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": "A 'azurerm_monitor_activity_log_alert' resource that monitors 'delete SQL server firewall rule' events should be defined",
...
at_least_one_valid_log_alert(resources) = {"result" : "has_valid_log"} {
	...
	resources[doc_index][x].criteria.operation_name == "Microsoft.Sql/servers/firewallRules/delete"
	...
	logs := {doc_index: filtered |
			...
			filtered := {key: resource |
				...
				resource.criteria.operation_name == "Microsoft.Sql/servers/firewallRules/delete"
...
get_results(value) = results {					# Case of one or more resources failing due to not setting an "action.action_group_id" field
	...
	results := [z |
		...
		z := {
			...
			"name" : name,
			"keyActualValue" : sprintf("The 'azurerm_monitor_activity_log_alert[%s]' resource monitors 'delete SQL server firewall rule' events but is missing an 'action.action_group_id' field", [name])
		}]
