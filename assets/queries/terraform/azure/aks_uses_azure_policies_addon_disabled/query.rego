...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("'azurerm_kubernetes_cluster[%s].addon_profile' is defined and not null", [name]),
...
CxPolicy[result] {
	...
	result := {
		...
		"issueType": "MissingAttribute",
		"keyExpectedValue": sprintf("'azurerm_kubernetes_cluster[%s].addon_profile.azure_policy' is defined and not null", [name]),
