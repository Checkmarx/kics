...
func requireEqualVulnerabilities(tb testing.TB, expected, actual []model.Vulnerability, dir string) {
	...
	for i := range expected {
		...
		require.Equal(tb, expectedItem.SimilarityID, actualItem.SimilarityID, "Invalid similarity id for query %s\n Expected: %s\n Actual: %s", dir, expectedItem.SimilarityID, actualItem.SimilarityID)
		require.Equal(tb, expectedItem.SearchLine, actualItem.SearchLine, "Invalid search line for query %s\n Expected: %s\n Actual: %s", dir, expectedItem.SearchLine, actualItem.SearchLine)
	}
}
