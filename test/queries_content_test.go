...
var (
	...
	requiredRegoResultProperties = []string{
		"documentId",
		"searchKey",
		"issueType",
		"issueType",
	...
	requiredQueryMetadataProperties = map[string]func(tb testing.TB, value interface{}, metadataPath string){
		"id": func(tb testing.TB, value interface{}, metadataPath string) {
			...
			require.True(tb, validUUID.MatchString(strings.TrimSpace(idValue)), "invalid UUID in query's metadata file %s", metadataPath)
		},
		"queryName": func(tb testing.TB, value interface{}, metadataPath string) {
			...
			require.NotNil(tb, nameValue, "invalid query name in query's metadata file %s", metadataPath)
		},
		"severity": func(tb testing.TB, value interface{}, metadataPath string) {
			...
			require.Contains(tb, severityList, strings.ToUpper(severityValue), "invalid severity in query's metadata file %s", metadataPath)
		},
		"category": func(tb testing.TB, value interface{}, metadataPath string) {
			...
			require.NotEmpty(tb, categoryValue, "empty category in query's metadata file %s", metadataPath)
		},
		"descriptionText": func(tb testing.TB, value interface{}, metadataPath string) {
			...
			require.NotEmpty(tb, descriptionValue, "empty description text in query's metadata file %s", metadataPath)
		},
	...
	for _, entry := range queries {
		t.Run(path.Base(entry.dir), func(t *testing.T) {
			...
			for k, validation := range requiredQueryMetadataProperties {
				value, ok := metadata[k]
				require.True(t, ok, "missing key '%s' in query's metadata file %s", k, metadataPath)
...
func testQueryHasGoodReturnParams(t *testing.T, entry queryEntry) {
	...
	inspector, err := engine.NewInspector(
		...
		func(ctx engine.QueryContext, trk engine.Tracker, v interface{}) (model.Vulnerability, error) {
			...
			for _, requiredProperty := range requiredRegoResultProperties {
...
func testMetadataFieldStringType(tb testing.TB, value interface{}, key, metadataPath string) string {
	...
	require.True(tb, ok, "wrong value type for key '5s' in query's metadata file %s", key, metadataPath)
	return stringValue
}

func testMetadataURL(tb testing.TB, url, metadataPath string) {
	require.True(tb, isValidURL(url), "invalid url in query's metadata file %s", metadataPath)
}
...
func testUnmarshalMetadata(tb testing.TB, entry queryEntry) (meta map[string]interface{}, metadataPath string) {
	...
	require.NoError(tb, err, "can't read query's metadata file %s", metadataPath)

	var metadata map[string]interface{}
	err = json.Unmarshal(content, &metadata)
	require.NoError(tb, err, "can't unmarshal query's metadata file %s", metadataPath)
