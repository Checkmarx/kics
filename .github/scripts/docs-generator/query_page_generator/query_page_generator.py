...
# Searches for "metadata.json" files within the inputed directory and returns a dictionary of dictionaries
...
def get_query_info(query_path : str, metadata_file = 'metadata.json', test_extensions = (".json", ".yaml", ".tf", ".dockerfile")):
    ...
    if not os.path.exists(metadata_file_path): return
    ...
    if not os.path.isdir(test_folder_path): return

    expected_result_file_path = os.path.join(test_folder_path, 'positive_expected_result.json')
    if not os.path.exists(expected_result_file_path): return
...
def format_positive_tests(positive_tests : dict) -> str:
    ...
    for idx, x in enumerate(positive_tests):
        ...
        # If the query has more than 3 tests, the remaining tests are placed in a drop down
        if idx <= 2:
            result += f'```{extension} title="{title}" hl_lines="{results_lines}"\n{code}\n```\n'
        else:
            result += f"<details><summary>{title}</summary>\n\n"
            result += f'```{extension} hl_lines="{results_lines}"\n{code}\n```\n'
            result += "</details>\n"
...
def generate_md_docs(queries_database : str, output_path : str, template_file_path = 'template.md'):
    ...
    # Check if the folder name is "queries"
    if folder_name == "queries":
        # Iterate over the subdirectories inside the "queries" folder
        for subdir in os.listdir(output_path):
            # Create the absolute path of the subdirectory
            subdir_path = os.path.join(output_path, subdir)
            # Check if it is a directory and delete it if it is
            if os.path.isdir(subdir_path):
                shutil.rmtree(subdir_path)

    if not os.path.exists(template_file_path): return
    ...
    for key, query_data in queries_database.items():
        ...
        query_doc = doc_template.replace("<QUERY_ID>", query_data.get('id')).replace(
        ...
        platform_folder_path = os.path.join(output_path,
                                            sanitize_filename(query_data.get('platform')) + "-queries",
                                            sanitize_filename(query_data.get("cloudProvider", "common")))
...
def main():
    ...
    parser.add_argument('-p', nargs=1, type=Path, dest='input_path', help='Folder path to read metadata.json', required=True)
    parser.add_argument('-o', nargs=1, type=Path, dest='output_path', help='Folder path to output documentation files', required=True)
    parser.add_argument('-f', nargs=1, type=str, dest='output_format', choices=['json', 'md'],
                        help='Documentation formats to be created, this script only supports json and md', required=True)
    parser.add_argument('--t', nargs=1, type=Path, dest='template_path', help='Template file path')
    ...
    if not input_path.exists(): return print("\033[31mError:\033[0m input_path doesn't exist in the operating system")
    ...
    if output_format == 'json':
        ...
        if not template_path.exists(): return print("\033[31mError:\033[0m template_path doesn't exist in the operating system")

        generate_md_docs(queries_database, str(output_path), str(template_path))
